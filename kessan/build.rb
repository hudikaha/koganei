...
html = <<~HTML
  ...
  <body>
    ...
    <script>
      ...
      function businessModeControls(parent){const mode=document.createElement('div');mode.className='business-mode';mode.innerHTML=`<label><input type="checkbox" ${businessView==='chart'?'checked':''}>事業を円グラフ化</label>`;mode.querySelector('input').onchange=event=>{businessView=event.target.checked?'chart':'list';if(event.target.checked){const section=parent.children.find(node=>node.id===selected);if(section){path=[...path,section];selected=null;setLocation(section.page,null)}}else if(current()?.level===4){const section=path.pop();selected=section.id;setLocation(section.page,null)}render(true)};return mode}
