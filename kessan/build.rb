...
html = <<~HTML
  ...
  <body>
    ...
    <script>
      ...
      function setLocation(page,jigyo,replace=false,section=null,view=null){const u=new URL(location.href);page?u.searchParams.set('page',page):u.searchParams.delete('page');jigyo?u.searchParams.set('jigyo',jigyo):u.searchParams.delete('jigyo');section?u.searchParams.set('section',section):u.searchParams.delete('section');view?u.searchParams.set('view',view):u.searchParams.delete('view');history[replace?'replaceState':'pushState']({},'',u)}
      ...
      function renderChart(nodes,parent,depth){
        ...
        const appendDetails=()=>{
          const chosenSection=parent?.level===3?(parent.children.find(node=>node.id===selected)||(path[depth]?.level===4?path[depth]:null)):null;if(parent?.level===3)section.append(businessModeControls(parent));if(chosenSection){if(businessView==='list')section.append(businessBreakdown(parent,chosenSection));return}
          if(!parent||(!parent.details.length&&!Number.isFinite(parent.detail_difference)))return;
          const details=document.createElement('div');details.className='details';details.innerHTML=`<h3>${kind==='expense'?'事業・内訳・個別支出':'決算書の備考'}</h3>`;
          ...
          if(parent.details.length){const list=document.createElement('div');list.className='details-list';parent.details.forEach(item=>{const project=document.createElement('details');project.className='project-detail';project.dataset.number=item.number||'';project.dataset.key=jigyoKey(account,path.slice(0,depth),item);project.addEventListener('toggle',()=>{if(restoring)return;if(project.open)setLocation(item.page||parent.page,project.dataset.key);else if(new URL(location.href).searchParams.get('jigyo')===project.dataset.key)setLocation(item.page||parent.page,null)});const summary=document.createElement('summary');summary.innerHTML=`事業 ${item.number||'—'}　${item.name}${item.department?` <span class="minor">（${item.department}）</span>`:''} <span class="detail-amount">${yen.format(item.amount)}円</span>`;project.append(summary);if(Number.isFinite(item.section_difference)&&item.section_difference!==0){const check=document.createElement('p'),diff=item.section_difference;check.className='detail-check warn';check.textContent=`注意：内訳合計が事業額と${yen.format(Math.abs(diff))}円${diff<0?'不足':'超過'}しています。`;project.append(check)}if(item.sections?.length){const sections=document.createElement('ul');sections.className='section-list';item.sections.forEach(part=>{const li=document.createElement('li');li.innerHTML=`<strong>内訳 ${part.number}　${part.name}</strong> <span class="detail-amount">${yen.format(part.amount)}円</span>`;if(part.items?.length){const items=document.createElement('ul');items.className='item-list';part.items.forEach(entry=>{const row=document.createElement('li');row.innerHTML=`${entry.name} <span class="detail-amount">${yen.format(entry.amount)}円</span>`;items.append(row)});li.append(items)}if(Number.isFinite(part.item_difference)&&part.item_difference!==0){const warning=document.createElement('p');warning.className='detail-check warn';warning.textContent=`注意：個別支出合計が内訳額と${yen.format(Math.abs(part.item_difference))}円${part.item_difference<0?'不足':'超過'}しています。`;li.append(warning)}sections.append(li)});project.append(sections)}list.append(project)});details.append(list)}section.append(details)
        };
      ...
      function renderCharts(){const box=$('charts');box.innerHTML='';renderChart(roots(),null,0);path.filter(node=>node.level<4).forEach((node,i)=>renderChart(node.children,node,i+1));if(current()?.level===4){const parent=path[path.length-2];box.append(businessBreakdown(parent,current()))}}
      function openNode(node,depth){selected=node.id;const targetPath=[...path.slice(0,depth),node],key=hierarchyKey(account,targetPath),section=node.level===4?node.code:null,view=section&&businessView==='chart'?'business':null;if(node.children.length||node.details.length||(node.level===4&&businessView==='chart')){path=targetPath;selected=null;setLocation(node.page,key,false,section,view);render(true)}else{setLocation(node.page,key,false,section,view);render();if(node.level===4)requestAnimationFrame(()=>document.querySelector('.business-breakdown')?.scrollIntoView({behavior:'smooth',block:'start'}))} }
      ...
      function applyUrl(){const params=new URL(location.href).searchParams,page=params.get('page'),key=params.get('jigyo'),sectionCode=params.get('section'),view=params.get('view'),jhit=key&&SEARCH.find(x=>x.jigyo===key);if(jhit){const resolvedPage=jhit.detail?.page||jhit.n.page;if(sectionCode&&!jhit.detail&&jhit.n.level===3){account=jhit.a;kind=jhit.k;path=[...jhit.trail,jhit.n];selected=null;const section=jhit.n.children.find(node=>String(node.code)===sectionCode);businessView=view==='business'?'chart':'list';if(section){if(businessView==='chart')path.push(section);else selected=section.id}setLocation(resolvedPage,jhit.jigyo,true,sectionCode,view);render(false);$('page-number').value=resolvedPage||'';return}setLocation(resolvedPage,jhit.jigyo,true);jhit.detail?openProject(jhit,false):showHit(jhit,false);$('page-number').value=resolvedPage||'';return}if(page){const hit=pageHit(page);if(!hit){render();$('page-status').textContent=`原本${page}ページは見つかりません`;return}const project=firstProject(hit,page);setLocation(page,project?.jigyo,true);project?openProject(project,false):showHit(hit);$('page-number').value=page;return}render()}
